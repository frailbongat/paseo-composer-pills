/**
 * Everything the ship pill knows, computed on the daemon.
 *
 ...
 * a silent pass, so the pill never offers a one-tap ship that `/ship` refuses.
 *
 ...
 * is waiting on a pill, so a tool that runs long is reported as undetermined
...
export async function readShipVerdictHandler(input: {
  ...
  try {
    ...
    console.error("[paseo-composer-pills] ship verdict failed", error);
